...
function createInstanceRepRev(classNameArg, replaceArg, reviveArg, options = {}) {
  ...
  const className = typeof classNameArg === 'function' ? classNameArg.name : classNameArg;

  // Prefer replaceArg, use classNameArg.replace instead if possible.
  const replace = typeof replaceArg === 'function' ? replaceArg : typeof classNameArg === 'function' && classNameArg.replace;

  // Prefer reviveArg, use classNameArg.revive instead if possible.
  const revive = typeof reviveArg === 'function' ? reviveArg : typeof classNameArg === 'function' && classNameArg.revive;
  return (0, _createRepRev.default)((key, value, owner) => {
